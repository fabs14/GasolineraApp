    ...
    private fun mostrarSucursales(tipo: String) {
        ...
        for ((idSucursal, nombreSucursal) in sucursales) {
            val btn = Button(this).apply {
                ...
                setOnClickListener {
                    val idSucursalCombustible = sucursalNegocio.obtenerIdSucursalCombustible(idSucursal, tipo)
                    val cantidadBombas = sucursalNegocio.obtenerCantidadBombas(idSucursal)

                    if (idSucursalCombustible != -1 && cantidadBombas != -1) {
                        val intent = Intent(this@SucursalPresentacion, ResultadoPresentacion::class.java).apply {
                            putExtra("idSucursal", idSucursal)
                            putExtra("idSucursalCombustible", idSucursalCombustible)
                            putExtra("cantidadBombas", cantidadBombas)
                        }
